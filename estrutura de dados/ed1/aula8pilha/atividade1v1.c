...
int main(int argc, char *argv[]){//procurar sobre os arg do parametro
    ...
    insere_pilha(&minha_pilha, 11); 
    insere_pilha(&minha_pilha, 22);
    insere_pilha(&minha_pilha, 33);
    insere_pilha(&minha_pilha, 44);
...
//o ponto usamos quando queremos acesar diretamente aql variavel, ja a setinha usamos no lugar do * quando estamos manipulando ponteiro
//---------------------------------
...
int topo(tipo_pilha pl){
    ...
    //verifica se pilha tem valor
    if(pl.cont > 0){
        aux = pl.vet_pilha[pl.cont-1];
    }
    else{
